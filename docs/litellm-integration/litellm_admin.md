...
- **Сложно кастомизировать фронтенд.** UI поставляется как предсобранная Next.js-статичка без исходников `.tsx` в пакете. Брендирование ограничено логотипом/темой; перестройка страниц — это фактически переписывание фронта с нуля.
